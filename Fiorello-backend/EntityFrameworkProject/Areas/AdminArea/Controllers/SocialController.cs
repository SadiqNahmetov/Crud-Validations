    ...
    public class SocialController : Controller
    {
        ...
        public async Task<IActionResult> Create(Social social)
        {
            try
            {
                ...
                bool isExistName = await _context.Socials.AnyAsync(m => m.Name.Trim() == social.Name.Trim());
                bool isExistUrl = await _context.Socials.AnyAsync(m => m.Url.Trim() == social.Url.Trim());
                if (isExistName  && isExistUrl)
                {
